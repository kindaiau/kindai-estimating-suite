...
// These representative sample results power the public interactive demo without an AI call.
...
// Public interactive sample. This endpoint does not accept files, free text, or call an AI model.
export const demoRouter = router({
  runDemo: publicProcedure.input(z.object({
    trade: z.enum(["cabinetry", "plumbing"]),
    ...
    const scenario = DEMO_SCENARIOS[input.trade] ?? DEMO_SCENARIOS.electrical;
    let totalMaterialsRetail = 0;
    let totalMaterialsTrade = 0;
    let totalLabourHours = 0;

    for (const item of scenario.sampleItems) {
      const wasteMultiplier = 1 + item.wasteFactor / 100;
      const quantityWithWaste = item.quantity * wasteMultiplier;
      totalMaterialsRetail += quantityWithWaste * item.retailPrice;
      totalMaterialsTrade += quantityWithWaste * item.tradePrice;
    ...
    return {
      ...
      items: scenario.sampleItems,
      assumptions: scenario.assumptions,
      planNotes: scenario.planNotes,
      pricing: {
        materialsCostRetail: Math.round(totalMaterialsRetail * 100) / 100,
        materialsCostTrade: Math.round(totalMaterialsTrade * 100) / 100,
        tradeSavings: Math.round((totalMaterialsRetail - totalMaterialsTrade) * 100) / 100,
