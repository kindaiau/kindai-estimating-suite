...
describe("Ebook Email Sequence", () => {
  ...
  describe("Day 4: Social Proof Email", () => {
    ...
    it("includes the estimator review checklist without fabricated proof", async () => {
      await sendEbookNurtureDay4({ to: "test@test.com", name: "Test User" });

      const call = mockSendEmail.mock.calls[0][0];
      expect(call.html).toContain("Four things to check");
      expect(call.html).not.toContain("40 minutes");
    });
  });
  ...
  describe("Day 7: ROI Calculator Email", () => {
    ...
    it("uses the reader's own baseline instead of invented ROI", async () => {
      await sendEbookNurtureDay7({ to: "test@test.com", name: "Test User" });

      const call = mockSendEmail.mock.calls[0][0];
      expect(call.html).toContain("your own numbers");
      expect(call.html).not.toContain("$41,400");
    });
  ...
  describe("Day 10: Last Chance Email", () => {
    ...
    it("includes the paid setup CTA instead of a trial", async () => {
      await sendEbookNurtureDay10({ to: "test@test.com", name: "Test User" });

      const call = mockSendEmail.mock.calls[0][0];
      expect(call.html).toContain("Founding Workflow Setup");
      expect(call.html).not.toContain("Pro Trial");
    });
  });
